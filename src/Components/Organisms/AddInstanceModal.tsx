...
export function AddInstanceModal({
  ...
  const [plannedDaysInput, setPlannedDaysInput] = useState("");
  ...
  const expectedProfit = parseFloat(expectedProfitInput) || 0;
  const plannedDays = parseInt(plannedDaysInput, 10) || 0;
  ...
  const isProfitValid = expectedProfit > 0;
  const isDaysValid = plannedDays >= 1 && plannedDays <= 365 && Number.isInteger(plannedDays);
  const canSubmit = isAmountValid && isProfitValid && isDaysValid && !loading;
  ...
  const handleSubmit = async () => {
    ...
    if (expectedProfit <= 0) {
      setError("Please enter a profit goal greater than $0.");
      return;
    }
    if (plannedDays < 1 || plannedDays > 365 || !Number.isInteger(plannedDays)) {
      setError("Please enter a whole number of days between 1 and 365.");
      return;
    }
    setLoading(true);
    try {
      const instanceId = await startInstance({
        ...
        expectedProfitUsdt: expectedProfit,
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 space-y-4">
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-xs font-bold text-foreground">Set your personal goal</p>
                  <p className="text-[11px] text-muted-foreground leading-relaxed mt-0.5">
                    This is just for you — it won't change how your bot trades, and your bot
                    won't stop early or late because of it. It simply shows you how you're tracking
              ...
              <div className="space-y-1.5">
                <label
                  htmlFor="instance-goal-profit-input"
                  className="text-[11px] font-mono font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"
                >
                ...
                <div className="relative">
                  ...
                  <input
                    id="instance-goal-profit-input"
                    type="number"
                    min={0.01}
                    step="any"
                    className={cn(
                      "w-full pl-7 pr-16 py-2.5 rounded-xl border bg-card/50 font-mono text-sm font-bold text-foreground outline-none transition-all",
                      expectedProfit > 0
                        ? "border-primary/40 focus:border-primary/70"
                        : "border-border/50 focus:border-primary/60"
                    )}
                    placeholder="e.g. 50"
                    value={expectedProfitInput}
                    onChange={(e) => setExpectedProfitInput(e.target.value)}
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-muted-foreground/60 font-mono">USDT</span>
                </div>
                {expectedProfitInput !== "" && expectedProfit <= 0 && (
                  <p className="text-[11px] text-rose-400 font-mono flex items-center gap-1">
                    <AlertTriangle className="w-3 h-3" />
                    Profit goal must be greater than $0.
                  </p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                <label
                  htmlFor="instance-goal-days-input"
                  className="text-[11px] font-mono font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"
                >
                ...
                <div className="relative">
                  <input
                    id="instance-goal-days-input"
                    type="number"
                    min={1}
                    max={365}
                    step={1}
                    className={cn(
                      "w-full pl-4 pr-14 py-2.5 rounded-xl border bg-card/50 font-mono text-sm font-bold text-foreground outline-none transition-all",
                      plannedDays >= 1 && plannedDays <= 365 && Number.isInteger(plannedDays)
                        ? "border-primary/40 focus:border-primary/70"
                        : "border-border/50 focus:border-primary/60"
                    )}
                    placeholder="e.g. 30"
                    value={plannedDaysInput}
                    onChange={(e) => setPlannedDaysInput(e.target.value)}
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-muted-foreground/60 font-mono">days</span>
                </div>
                {plannedDaysInput !== "" && (plannedDays < 1 || plannedDays > 365 || !Number.isInteger(plannedDays)) && (
                  <p className="text-[11px] text-rose-400 font-mono flex items-center gap-1">
                    <AlertTriangle className="w-3 h-3" />
